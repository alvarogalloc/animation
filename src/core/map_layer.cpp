...
private:
  ...
  public:
    ...
    {
      if (registerAnimation) { m_activeAnimations.clear(); }
          ...
          for (auto x = xPos; x < xPos + chunkTileCount.x; ++x)
          {
                ...
                && m_chunkTileIDs[idx].ID <= ca->m_lastGID)
            {
              ...
              sf::Vector2f tileOffset(static_cast<float>(x) * mapTileSize.x,
                static_cast<float>(y) * mapTileSize.y + mapTileSize.y
                  - ca->tileSetSize.y);
              ...
              Tile tile = { sf::Vertex(tileOffset - getPosition(),
                              m_chunkColors[idx],
                              tileIndex),
    ...
    void maybeRegenerate(bool refresh)
    {
      if (refresh)
      {
        for (const auto &ca : m_chunkArrays) { ca->reset(); }
        generateTiles();
      }
    }
  ...
  private:
    ...
    public:
      ...
      {
        for (const auto &v : tile) { m_vertices.push_back(v); }
      }
    ...
    void draw(sf::RenderTarget &rt, sf::RenderStates states) const override
    {
      ...
      for (const auto &a : m_chunkArrays) { rt.draw(*a, states); }
    }
  };
  ...
  void createChunks(const tmx::Map &map, const tmx::TileLayer &layer)
  {
    ...
    for (auto y = 0u; y < m_chunkCount.y; ++y)
    {
      sf::Vector2f tileCount(
        m_chunkSize.x / tileSize.x, m_chunkSize.y / tileSize.y);
  ...
  void draw(sf::RenderTarget &rt, sf::RenderStates states) const override
  {
    ...
    for (const auto &c : m_visibleChunks) { rt.draw(*c, states); }
  }
};
